    ...
    methods (Static)
        function plotFigures = plot(name, figures)
            switch name
                ...
                case "ErodedImage"
                    ImagePreProcessingTuningPlot.plotDetail(5, 4, 17, figures, name, true, true);
                otherwise
            end 
        end

        function plotDetail(row, column, index, figures, name, showSegmentedCharacter, extraProcessing)
            if nargin < 7 || isempty(extraProcessing)
